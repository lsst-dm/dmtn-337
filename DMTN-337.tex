...
\date{\vcsDate}
\author{Tatiana Acero-Cuellar, Federica B. Bianco, Eric C. Bellm}
...
The set of weights that generate the reliability score has been updated three times; for each update, the model architecture remains the same, but a different dataset was used. Each version of the reliability model is detailed in \autoref{tab:model_version}, \autoref{tab:model_version_cont}.
...
\begin{table}[htb!]
    ...
    \hline
        ...
        0.1 &tac\_cnn\_comcam\_2025-02-18&DC2 + Comcam + injections\\ \hline
        0.2 &tac\_cnn\_lsstcam\_2026-02-13&(DC2 + Comcam + injections) + zooniverse \\\hline
0.3&tac\_cnn\_lsstcam\_2026-02-26&(DC2 + Comcam + injections) + zooniverse + Comcam + injections \\ \hline
    \end{tabular}
    \caption{Model version. The model 0.1 represents the base model, version 0.2 and 0.3 were obtained by fine-tuning the base model. The training inputs in parentheses are data used to train the base model, but were not directly used to fine-tune the respective model.}
...
\begin{table}[htb!]
    ...
    \hline
        ...
        0.1 &from September 2025&0.1&DP1 model \\ \hline
        0.2 &18 February 2026&0.1 (before Feb 24, 2026) 0.5 (after Feb 24, 2026)& \\\hline
        0.3&March 2026&0.5&DP2 model \\ \hline
    \end{tabular}
    \caption{Model version continuation}
...
\subsection{Data for v0.2 and DP2 model}
...
Rubin Difference Detectives was launched on November 11th, 2025, with 242,391 DIA Sources (a subject in Zooniverse terms) for the Zooniverse community to label. The distribution of the DIA sources per catalog is described in \autoref{tab:list_data}.
...
\autoref{fig:gt_snr_byset} showed that low SNR tends to be classified as Bogus, regardless of the type of source (gaia, galaxy, reliability\_cuts, ss, or tns).
...
% Using these volunteer classifications, we constructed balanced datasets (same amount of Real and Bogus) for model training.
...
The behavior of each model against SNR for a fake injection run outside of the training data set is shown in \autoref{fig:cosmos_compare_models}. DP1 model was trained only using fakes; v0.2 was only trained with real data without fake injection, and the DP2 model was trained with both fakes and real data. 
...
The DP1 model achieved a purity of 98.90\% and completeness of 97.00\% in the test dataset.
...
The DP1 model tested against a fake injection run outside of the training data set is shown in \autoref{fig:cosmos_dp1}.
\begin{figure}
    \centering
...
The DP2 model reached a purity $\geq 95\%$ and completeness $\geq 90\%$ with threshold values between $0.808 \leq$threshold $\leq 0.833$ for the test data without fake injections. For the LSSTComcam fakes test data, the model reached a purity $\geq 95\%$ and completeness $\geq 90\%$, which are obtained with values between $0.129 \leq $ threshold $\leq 0.899$. \autoref{fig:dp2_performance} shows the full purity and completeness curves.
