...
The document defines the model versioning scheme for present and future reference and recaps all existing model versions, including the models used for DP1, DP2, and the initial set of nightly alerts. It describes the data used for each version, including how the labels obtained from the initial release of the Zooniverse Citizen Science Project 'Rubin Difference Detectives' were analyzed and used. The document also defines the reliability cutoff currently applied to all alerts, and presents how each model version behaves on fake injections and a random subset of recent LSSTCam detections.
}
...
\setDocChangeRecord{%
  ...
  \addtohist{2}{2026-07-09}{v0.3 deployment}{Bellm}
  \addtohist{3}{2026-07-23}{DRP model usage DP2}{Acero-Cuellar}
}
...
\begin{table}[htb!]
    ...
    \hline
        ...
        0.1 &September 2025&0.1&DP1 \& DP2 model \\ \hline
        0.2 &18 February 2026&0.1 (before Feb 24, 2026) 0.5 (after Feb 24, 2026)& \\\hline
        0.3& 9 July 2026 &0.5& \\ \hline
    \end{tabular}
	\caption{Model version continued. The AP reliability cutoff is implemented to mitigate the impact on APDB when generating alerts. The AP and DRP pipelines worked independently; each one can use a different model version. \V2 was used for initial public alerts. The \V1 model was used during broker integration tests; these alerts can be distinguished by their date range.  Starting around the \V3 deployment date, an explicit \texttt{reliabilityVersion} field in \texttt{DiaSource} records indicates the model used.}
...
The data to train the model has been in constant evolution, adjusting it every time to the most recent status of the telescope and data pipelines. For each model update, a specific dataset was used for training. Here, we describe the three datasets used for \V1-\V3 models.

\subsection{Data for \V1 (DP1 \& DP2) model: LSSTComCam data}

The ML-Reliability model was initially trained with 89,066 \texttt{Real} and the same amount of \texttt{Bogus} labels, extracted from the Data Challenge 2 simulations \cite[DC2,][]{2021arXiv210104855L}, plus random injections of stars (i.e., PSFs at random locations without Galaxy associations) to increase the number of \texttt{Real}. Once the LSSTComCam data became available, the model was fine-tuned on a subset of that data containing 183,046 postage stamps with PSF injections. This fine-tuned model was the one used to generate reliability scores for Data Preview 1 \cite[DP1,][]{RTN-095} and DP2 \citep{RTN-115}, and is tagged \V1. 

\subsection{Data for \V2 and \V3 model: LSSTCam and Zooniverse}

The model was retrained (fine-tuned) to improve performance on the commissioning LSSTCam data, leading to model version \V3. The \V3 model, as of July 2026, is being used for the AP pipeline and the production of alerts. The training of \V3, and \V2 relies partially and totally, respectively, on real data (no fake injection) and \texttt{Real}/\texttt{Bogus} labels obtained by the Zooniverse citizen scientists through the `Rubin Difference Detectives' project. 
...
\begin{figure}[htb!]
   ...
   \caption{DIA Aperture Flux vs ScienceFlux - TemplateFlux colored by the reliability score with a cutoff for each model version for a random set of DIA sources. The cutoff is only implemented for the alerts.}
...
\subsection{Model \V1 (deployed on DP1 \& DP2)}\label{sec:v01}
...
The \V1 model was deployed on DP1 and DP2 data. The model had achieved a purity of 98.90\% and completeness of 97.00\% on the test dataset generated from the same collection from which the training and validation data were produced (DC2 simulations, LSSTComCam images with and without injections, see \autoref{sec:data}).
...
\includegraphics[width=0.6\linewidth]{figures/cosmos_dp1_model.png}
    \caption{Performance of the \V1 (DP1 \& DP2) model on fake sources injected into the COSMOS field LSSTCam images.}
...
\begin{table}[ht]
\centering
\caption{Classification Results for Solar System Transient and Variable objects using the \V1 model with a reliability threshold of 0.5.}
...
\begin{figure*}[htb!]
   \centering
   \includegraphics[width=0.45\linewidth]{figures/stable_v03_report_new_model.png}
    \includegraphics[width=0.45\linewidth]{figures/ComCam_v03_report_new_model.png}
...
\autoref{fig:v03_performance} shows the corresponding purity and completeness curves.

The \V3 model performance on the COSMOS injection is shown in \autoref{fig:cosmos_v03}. 84\% Purity and 84\% Completeness are achieved at $\rel=0.785$.


\begin{figure}
    \centering
\includegraphics[width=0.6\linewidth]{figures/cosmos_v03_report_new_model.png}
    \caption{Performance of the \V3 model on fake sources injected into the COSMOS field LSSTCam images.}
    \label{fig:cosmos_v03}
\end{figure}
...
The full Purity and Completeness curves per data origin as measured on the original test data are shown in \autoref{fig:v03_performance_byset}.
...
\begin{figure}[htb!]
    \centering
    \includegraphics[width=1\linewidth]{figures/stable_byset_v03_report_new_model.png}
    \caption{Purity and completeness per data origin for test data without fake injection (Zooniverse labels).}
    \label{fig:v03_performance_byset}
\end{figure}
